...
function loadSettings() {
  ...
  return { maxBudgetPerTrade: 0.1, slippage: 1 };
}
...
function saveSkills() {
  try {
    ...
    for (const [name, skill] of skills) {
      list.push({ name, url: skill.url, content: skill.content, active: skill.active });
    }
...
async function chat(userId, userMessage) {
  ...
  const systemPrompt = 'Kamu adalah HervBot, AI trading agent crypto di Solana.' +
    ...
    (skillsContext ? '\n\nSkill aktif:\n' + skillsContext : '') +
    '\n\nJangan mengarang URL GitHub. Kalau pengguna minta cari skill, suruh pakai perintah /cari [keyword] untuk hasil nyata. Jangan gunakan Markdown. Teks biasa saja. Bahasa Indonesia santai.';
  try {
...
async function autoLoadSkills() {
  ...
  for (const s of saved) {
    skills.set(s.name, { url: s.url, content: s.content, active: s.active });
  }
  console.log('Auto-loaded ' + saved.length + ' skills from storage');
}

autoLoadSkills().then(() => {
  app.listen(config.PORT, () => {
    console.log('HervBot running on port ' + config.PORT);
    console.log('Settings: budget=$' + settings.maxBudgetPerTrade + ' slippage=' + settings.slippage + '%');
...
async function githubPushFile(repoName, filePath, content, commitMessage) {
  try {
    ...
    const owner = process.env.GITHUB_USERNAME || 'herviana';
    if (!token) throw new Error('GITHUB_TOKEN belum diset di Railway');

    // Cek apakah repo sudah ada, kalau belum buat dulu
    ...
    if (!repoCheck.ok) {
      ...
      const createRepo = await fetch('https://api.github.com/user/repos', {
        ...
        body: JSON.stringify({
          ...
          private: false,
          auto_init: true,
          description: 'HervBot Skill - ' + repoName,
        }),
      });
      if (!createRepo.ok) throw new Error('Gagal buat repo: ' + await createRepo.text());
      // Tunggu repo siap
    ...
    // Push file
    const body = {
      message: commitMessage || 'Update ' + filePath + ' via HervBot',
    ...
    if (!pushRes.ok) throw new Error('Gagal push file: ' + await pushRes.text());
...
async function fetchSkillMd(githubUrl) {
  try {
    let url = githubUrl.trim().replace(/\/$/, '');
    url = url.replace('https://', '').replace('http://', '');
    const parts = url.replace('github.com/', '').split('/');
    ...
    for (const branch of branches) {
      ...
      try {
        const res = await fetch(rawUrl);
        if (res.ok) return { success: true, content: await res.text() };
      } catch (e) {}
    }
    return { success: false, error: 'SKILL.md tidak ditemukan' };
  } catch (e) {
    return { success: false, error: e.message };
  }
}
...
bot.onText(/\/github_push (.+) (.+)/, async (msg, match) => {
  ...
  bot.sendMessage(chatId, 'Mengambil skill dari ' + sourceUrl + '...');

  const skillResult = await fetchSkillMd(sourceUrl);
  if (!skillResult.success) {
    bot.sendMessage(chatId, 'Gagal ambil SKILL.md: ' + skillResult.error);
    return;
  }

  // Personalisasi konten
  const owner = process.env.GITHUB_USERNAME || 'herviana';
  let content = skillResult.content;
  content = content.replace(/author:\s*.+/gi, 'author: ' + owner);

  bot.sendMessage(chatId, 'Pushing ke repo ' + repoName + '...');

  // Push SKILL.md
  const pushSkill = await githubPushFile(repoName, 'SKILL.md', content, 'Add SKILL.md via HervBot');
  if (!pushSkill.success) {
    bot.sendMessage(chatId, 'Gagal push SKILL.md: ' + pushSkill.error);
    return;
  }

  // Push README.md
  const readme = '# ' + repoName + '\n\nSkill untuk HervBot.\n\nDiduplikat dari: ' + sourceUrl + '\nOleh: ' + owner;
  await githubPushFile(repoName, 'README.md', readme, 'Add README.md via HervBot');

  bot.sendMessage(chatId,
    'Berhasil! Skill sudah dipush ke GitHub!\n\n' +
    'Repo: github.com/' + owner + '/' + repoName + '\n\n' +
    'Install skill ini:\n' +
    '/install https://github.com/' + owner + '/' + repoName
  );
});

// ── MODIFIKASI DAN PUSH ──
bot.onText(/\/modif_push (.+) (.+)/, async (msg, match) => {
  const chatId = msg.chat.id;
  const repoName = match[1].trim();
  const sourceUrl = match[2].trim();

  const pending = modifPending.get(chatId);
  if (!pending) {
    bot.sendMessage(chatId, 'Tidak ada modifikasi pending.\n\nCara pakai:\n1. Minta modifikasi skill dulu\n2. Konfirmasi dengan /modif_push nama-repo url-sumber');
    return;
  }

  bot.sendMessage(chatId, 'Pushing skill yang sudah dimodifikasi ke ' + repoName + '...');

  const pushResult = await githubPushFile(repoName, 'SKILL.md', pending.content, 'Modified skill via HervBot');
  if (!pushResult.success) {
    bot.sendMessage(chatId, 'Gagal push: ' + pushResult.error);
    return;
  }

  const owner = process.env.GITHUB_USERNAME || 'herviana';
  const readme = '# ' + repoName + '\n\nSkill untuk HervBot.\n\nDimodifikasi dari: ' + sourceUrl + '\nOleh: ' + owner;
  await githubPushFile(repoName, 'README.md', readme, 'Add README.md');

  modifPending.delete(chatId);

  bot.sendMessage(chatId,
    'Berhasil! Skill sudah dipush!\n\n' +
    'Repo: github.com/' + owner + '/' + repoName + '\n\n' +
    'Install:\n' +
    '/install https://github.com/' + owner + '/' + repoName
  );
...
async function searchGithubSkills(keyword) {
  try {
    const token = process.env.GITHUB_TOKEN;
    const query = keyword + ' SKILL.md in:path language:markdown';
    const url = 'https://api.github.com/search/repositories?q=' + encodeURIComponent(keyword + ' skill trading crypto') + '&sort=stars&order=desc&per_page=5';
    ...
    if (!res.ok) throw new Error('GitHub search gagal: ' + await res.text());
    const data = await res.json();
    ...
    // Filter repo yang punya SKILL.md
    const results = [];
...
bot.onText(/\/cari (.+)/, async (msg, match) => {
  ...
  bot.sendMessage(chatId, 'Mencari skill "' + keyword + '" di GitHub...');
  
  const result = await searchGithubSkills(keyword);
  if (!result.success) {
    bot.sendMessage(chatId, 'Gagal search: ' + result.error);
    return;
  }
  
  if (result.results.length === 0) {
    bot.sendMessage(chatId, 'Tidak ada skill ditemukan untuk kata kunci: ' + keyword);
    return;
  }
...
';
    msg2 += 'URL: ' + r.url + '
';
    msg2 += 'Deskripsi: ' + r.description + '
';
    msg2 += 'Stars: ' + r.stars + ' | Update: ' + r.updatedAt + '
';
    msg2 += r.hasSkill ? 'SKILL.md: Ada
' : 'SKILL.md: Tidak ada
';
    msg2 += '
';
  ...
  msg2 += 'Install skill (ada SKILL.md):
/install https://github.com/username/repo

';
  msg2 += 'Duplikat ke repo kamu:
/github_push nama-repo-baru https://github.com/username/repo';
  
  bot.sendMessage(chatId, msg2);
});

const modifPending = new Map();
